...
####################################################################
## PixPLot D3.js and Image Preprocessing
...
flags.DEFINE_string('layout', 'umap', 'The layout method to use {umap|tsne|pca}')
FLAGS = flags.FLAGS
...
################################################################################
## Gets image vectors by running Inception and
## saving object detections as feature vectors
    ...
    with tf.Session() as sess:
      for image_index, image in enumerate(self.image_files):
        try:
          ...
          print(' * image', get_ascii_chars(image))
...
## TSNE + UMAP
  ...
  def build_model(self, image_vectors):
    ...
    if self.method == 'tsne':
        ...
        print(' * PCA')
        np.set_printoptions(suppress=True)
        return PCA().fit_transform( np.array(image_vectors) )
