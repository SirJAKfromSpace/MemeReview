...
def run_inference_on_images(image_list, output_dir):
  ...
  with tf.Session() as sess:
    ...
    ############### looping through each image in arg list #####################
...
### Downloads the Inception Model if not already cached ###
...
def main(_):
  ...
  if len(sys.argv) < 2:
    ...
    ## makes a directory for saving the final node weights from the NN as vectors
    output_dir = "image_vectors"
    if not os.path.exists(output_dir):
      os.makedirs(output_dir)

    # not sure what a glob is
    images = glob.glob(sys.argv[1])
     #this is the real function that calls tensorflow and the image processor
    image_to_labels = run_inference_on_images(images, output_dir)

    # label and vector file writer
